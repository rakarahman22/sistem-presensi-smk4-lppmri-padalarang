...
class WaliImportController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $msg = "Import selesai: {$imported} wali siswa berhasil diimpor";
            if ($skipped > 0) {
                $msg .= ", {$skipped} baris dilewati (username duplikat atau data tidak lengkap).";
            } else {
                $msg .= '.';
            }

            return redirect()->route('admin.wali')->with('success', $msg);
    ...
    /**
     * GET /admin/wali/template
     */
    public function template()
    {
        $spreadsheet = new Spreadsheet();
        $sheet       = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Import Wali Siswa');

        // ── Definisi kolom ────────────────────────────────────────────────────
        $columns = [
            ['key' => 'nama_wali', 'label' => 'Nama Wali *', 'width' => 32,  'required' => true],
            ['key' => 'username',  'label' => 'Username',    'width' => 24,  'required' => false],
            ['key' => 'password',  'label' => 'Password',    'width' => 22,  'required' => false],
            ['key' => 'no_telp',   'label' => 'No. Telepon', 'width' => 20,  'required' => false],
        ];
        ...
        // ── Baris 2: Key teknis (heading row yang dibaca WaliSiswaImport) ─────
        foreach ($columns as $ci => $col) {
            $colNum = $ci + 1;
        ...
        $notes = [
            ...
            ['🔁  Username default = nama.lowercase (cth: "budi.santoso") jika kolom username kosong.',         'FF1E40AF'],
            ['🔐  Password default = "wali1234" jika kolom password kosong.',                                   'FF1E40AF'],
            ['📱  No. Telepon: format bebas, cth: 081234567890 atau +6281234567890.',                           'FF374151'],
            ['🚫  Baris dengan username yang sudah ada di sistem akan dilewati otomatis.',                       'FF6B7280'],
        ];
        ...
        $tmpFile = tempnam(sys_get_temp_dir(), 'tpl_wali_');
        $writer->save($tmpFile);

        return response()->download($tmpFile, 'template_import_wali_siswa.xlsx', [
