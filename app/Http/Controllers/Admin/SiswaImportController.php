...
class SiswaImportController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $msg = "Import selesai: {$imported} siswa berhasil diimpor";
            if ($skipped > 0) {
                $msg .= ", {$skipped} baris dilewati (NIS duplikat atau data tidak lengkap).";
            } else {
                $msg .= '.';
            }
    ...
    public function template()
    {
        ...
        // ── Definisi kolom ────────────────────────────────────────────────────
        $columns = [
            ['key' => 'nis',        'label' => 'NIS *',        'width' => 18,  'required' => true],
            ['key' => 'nama_siswa', 'label' => 'Nama Siswa *', 'width' => 30,  'required' => true],
            ['key' => 'username',   'label' => 'Username',     'width' => 22,  'required' => false],
            ['key' => 'password',   'label' => 'Password',     'width' => 22,  'required' => false],
            ['key' => 'nama_kelas', 'label' => 'Nama Kelas',   'width' => 20,  'required' => false],
            ['key' => 'nama_wali',  'label' => 'Nama Wali',    'width' => 28,  'required' => false],
        ];
        ...
        // ── Baris 2: Key teknis (heading row yang dibaca SiswaImport) ────────
        foreach ($columns as $ci => $col) {
            $colNum = $ci + 1;
        ...
        $examples = [
            ['2024001', 'Budi Santoso',   'budi.santoso', 'budi2024', 'TKJ 1',  'Santoso'],
        ...
        $notes = [
            ...
            ['🔁  Username default = NIS jika kosong.  Password default = NIS jika kosong.', 'FF1E40AF'],
            ['📌  Nama Kelas: isi sesuai nama kelas di sistem, cth: "TKJ 1" atau "X TKJ 1".', 'FF374151'],
            ['👤  Nama Wali: isi sesuai nama wali yang sudah terdaftar di sistem.', 'FF374151'],
            ['🚫  Baris dengan NIS yang sudah ada di sistem akan dilewati otomatis.', 'FF6B7280'],
        ];
        ...
        $tmpFile = tempnam(sys_get_temp_dir(), 'tpl_siswa_');
        $writer->save($tmpFile);

        return response()->download($tmpFile, 'template_import_siswa.xlsx', [
